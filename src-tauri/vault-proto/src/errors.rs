...
pub enum ErrorCode {
    ...
    // Provider wire codes (§11.3 error table; returned by
    // vault-provider-core, surfaced to the helper through main).
    AuthInvalid,
    ManifestInvalid,
    IndexInvalid,
    RegistryInvalid,
    CheckpointMismatch,
    BlobMissing,
    /// `413`: a blob or bootstrap payload over its §11.2 cap.
    TooLarge,
...
impl ErrorCode {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            ErrorCode::IndexInvalid => "INDEX_INVALID",
            ErrorCode::RegistryInvalid => "REGISTRY_INVALID",
