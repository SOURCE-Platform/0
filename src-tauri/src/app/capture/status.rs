...
pub async fn build_channel_statuses(state: &AppState) -> Result<Vec<ChannelStatusDto>, String> {
    ...
    let channels = vec![
        ...
        // Video evidence is disabled: no MP4 segments are written. OCR
        // screenshots are deleted after reading, so video_segments stays
        // empty and this channel reports off once existing rows are cleared.
