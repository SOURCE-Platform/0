pub use crate::core::video_encoder_types::{
    CompressionQuality, Result, VideoCodec, VideoEncoderError, VideoSegment,
};
...
impl VideoEncoder {
    ...
    fn encode_frames_sync(
        ...
        println!(
            "VideoEncoder: Encoding {} frames to {:?}",
            frames.len(),
            output_path
        );
        ...
        println!(
            ...
            hardware_acceleration, platform
        );
        ...
        let first_frame = frames
            .first()
            .ok_or_else(|| VideoEncoderError::EncodingFailed("No frames to encode".to_string()))?;
        ...
        let mut encoder =
            match FFmpegEncoder::new(output_path, width, height, fps, &codec_name, crf) {
                Ok(enc) => {
                ...
                Err(e) if hardware_acceleration => {
                    ...
                    FFmpegEncoder::new(output_path, width, height, fps, software_codec, crf)
                        .map_err(|e| {
                            VideoEncoderError::FFmpeg(format!(
                ...
                Err(e) => {
                    return Err(VideoEncoderError::FFmpeg(format!(
        ...
        for (i, frame) in frames.iter().enumerate() {
            encoder.encode_frame(frame).map_err(|e| {
                VideoEncoderError::FFmpeg(format!("Frame {} encoding failed: {}", i, e))
            })?;
        }
...
mod tests {
    ...
    async fn test_encode_single_frame() {
        let encoder =
            VideoEncoder::new(VideoCodec::H264, CompressionQuality::Medium, false).unwrap();
    ...
    async fn test_encode_multiple_frames() {
        let encoder = VideoEncoder::new(VideoCodec::H264, CompressionQuality::High, false).unwrap();
    ...
    async fn test_encode_empty_frames() {
        let encoder = VideoEncoder::new(VideoCodec::H264, CompressionQuality::Low, false).unwrap();
    ...
    async fn test_encode_frame_stream() {
        ...
        let result = encoder
            .encode_frame_stream(rx, output_path.clone(), 15)
            .await;
    ...
    fn test_codec_names() {
        ...
        assert_eq!(
            codec.to_ffmpeg_codec_name(true, "macos"),
            "h264_videotoolbox"
        );
